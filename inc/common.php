...
function pageinfo() {
    ...
    global $conf; // added by lwf

    global $USERINFO; // added by lwf
    ...
    //msg($_SESSION['nologin_account']."/".$conf['nologin_maxaccesspages']);
    //echo $_SESSION['nologin_account']."/".$conf['nologin_maxaccesspages'];

    if (isset($conf['nologin_maxaccesspages']) && $conf['nologin_maxaccesspages']>0) {

        $acl = $info['perm'];

        if (empty($USERINFO['name']) && $acl > AUTH_NONE && !isRobot()) {

            //session_start();

            if (!isset($_SESSION['nologin_account'])) {

                $_SESSION['nologin_account'] = 1;

            } else {

                $_SESSION['nologin_account'] = $_SESSION['nologin_account'] +1;
