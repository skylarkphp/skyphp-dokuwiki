...
/**
 * Class action_plugin_interactive
 */
class action_plugin_interactive extends DokuWiki_Action_Plugin{

    /** @var helper_plugin_avatar */
    var $avatar = null;
    var $style = null;
    var $use_avatar = null;
    /** @var helper_plugin_interactive */
    var $helper = null;
    ...
    public function __construct() {
        $this->helper = plugin_load('helper', 'interactive');
    }
    ...
    public function handle_ajax_call(Doku_Event $event, $params) {
        if($event->data != 'interactive_preview') return;
    ...
    /**
     * Adds a TOC item if a interactive exists
     *
     * @author Michael Klier <chi@chimeric.de>
     *
     * @param Doku_Event $event
    ...
    public function handle_toc_render(Doku_Event $event, $params) {
        global $ACT;
        if($this->_hasInteractive($title) && $event->data && $ACT != 'admin') {
            $tocitem = array( 'hid' => 'interactive__section',
                              'title' => $this->getLang('interactive'),
                              'type' => 'ul',
                              'level' => 1 );
    ...
    /**
     * Modify Tollbar for use with interactive plugin
     *
     * @author Michael Klier <chi@chimeric.de>
     *
     * @param Doku_Event $event
    ...
    public function handle_toolbar_define(Doku_Event $event, $param) {
        ...
        if($this->_hasInteractive($title) && $this->getConf('wikisyntaxok')) {
            $toolbar = array();
    ...
    /**
     * Dirty workaround to add a toolbar to the interactive plugin
     *
     * @author Michael Klier <chi@chimeric.de>
     *
     * @param Doku_Event $event
    ...
    public function handle_act_preprocess(Doku_Event $event, $param) {
        ...
        if ($event->data != 'show' && $event->data != 'interactive_unsubscribe' && $event->data != 'interactive_confirmsubscribe') return;

        if ($event->data == 'interactive_unsubscribe' or $event->data == 'interactive_confirmsubscribe') {
            ...
            switch ($_REQUEST['comment']) {
                case 'add':
                    ...
                    } elseif((isset($_SERVER['REMOTE_USER']) && $this->getConf('adminimport') && $this->helper->isInteractiveMod()) || !isset($_SERVER['REMOTE_USER'])) {
                    ...
                    $repl = $_REQUEST['reply'];
                    if($this->getConf('moderate') && !$this->helper->isInteractiveMod()) {
                        $comment['show'] = false;
                    } else {
                        $comment['show'] = true;
                    }
    ...
    /**
     * Checks config settings to enable/disable interactives
     *
     * @return bool
     */
    public function isInteractiveEnabled() {
        global $INFO;
    ...
    protected function _show($reply = null, $edit = null) {
        ...
        if (!@file_exists($file) && !$this->isInteractiveEnabled()) return false;
        ...
        } elseif (!@file_exists($file) && $this->isInteractiveEnabled() && $INFO['exists']) {
            // set status to show the comment form
            $data['status'] = 1;
            $data['number'] = 0;
        }

        // show interactive wrapper only on certain circumstances
        ...
        if($cnt > 1 || ($cnt == 1 && $data['comments'][$keys[0]]['show'] == 1) || $this->getConf('allowguests') || isset($_SERVER['REMOTE_USER'])) {
            $show = true;
            // section title
            $title = ($data['title'] ? hsc($data['title']) : $this->getLang('interactive'));
            ptln('<div class="comment_wrapper" id="comment_wrapper">'); // the id value is used for visibility toggling the section
            ptln('<h2><a name="interactive__section" id="interactive__section">', 2);
            ptln($title, 4);
            ptln('</a></h2>', 2);
        ...
        if($this->getConf('visibilityButton')) {           
            // print the hide/show interactive section button
    ...
    protected function _add($comment, $parent) {
        ...
        // get interactive meta file name
        $file = metaFN($ID, '.comments');
    ...
    public function save($cids, $raw, $act = NULL) {
        ...
        foreach ($cids as $cid) {
            ...
            if (($user != $_SERVER['REMOTE_USER']) && (!$this->helper->isInteractiveMod())) return false;
    ...
    protected function _print($cid, &$data, $parent = '', $reply = '', $visible = true) {
        ...
        if (!$comment['show']) {                            // comment hidden
            if ($this->helper->isInteractiveMod()) $hidden = ' comment_hidden';
            else return true;
        } else {
            $hidden = '';
        }
    ...
    protected function _print_comment($cid, &$data, $parent, $reply, $visible, $hidden) {
        ...
        if ($visible) {
            ...
            if ((($user == $_SERVER['REMOTE_USER']) && ($user != '')) || ($this->helper->isInteractiveMod())) {
    ...
    /**
     * Show the button which toggle the visibility of the interactive section
     */
    protected function _print_toggle_button() {
        ptln('<div id="toggle_button" class="toggle_button" style="text-align: right;">');
        ptln('<input type="submit" id="interactive__btn_toggle_visibility" title="Toggle Visibiliy" class="button" value="'.$this->getLang('toggle_display').'">');
        ptln('</div>');
    }
    ...
    protected function _form($raw = '', $act = 'add', $cid = NULL) {
        ...
        <div class="comment_form">
          <form id="interactive__comment_form" method="post" action="<?php echo script() ?>" accept-charset="<?php echo $lang['encoding'] ?>">
            <div class="no">
        ...
        if(!$_SERVER['REMOTE_USER'] or ($this->getConf('adminimport') && $this->helper->isInteractiveMod())) {
        ?>
              <input type="hidden" name="user" value="<?php echo clientIP() ?>" />
              <div class="comment_name">
                <label class="block" for="interactive__comment_name">
                  <span><?php echo $lang['fullname'] ?>:</span>
                  <input type="text" class="edit<?php if($_REQUEST['comment'] == 'add' && empty($_REQUEST['name'])) echo ' error'?>" name="name" id="interactive__comment_name" size="50" tabindex="1" value="<?php echo hsc($_REQUEST['name'])?>" />
                </label>
              </div>
              <div class="comment_mail">
                <label class="block" for="interactive__comment_mail">
                  <span><?php echo $lang['email'] ?>:</span>
                  <input type="text" class="edit<?php if($_REQUEST['comment'] == 'add' && empty($_REQUEST['mail'])) echo ' error'?>" name="mail" id="interactive__comment_mail" size="50" tabindex="2" value="<?php echo hsc($_REQUEST['mail'])?>" />
                </label>
              </div>
        <?php
        }
        ...
        ?>
              <div class="comment_url">
                <label class="block" for="interactive__comment_url">
                  <span><?php echo $this->getLang('url') ?>:</span>
                  <input type="text" class="edit" name="url" id="interactive__comment_url" size="50" tabindex="3" value="<?php echo hsc($_REQUEST['url'])?>" />
                </label>
              </div>
        <?php
        }
        ...
        ?>
              <div class="comment_address">
                <label class="block" for="interactive__comment_address">
                  <span><?php echo $this->getLang('address') ?>:</span>
                  <input type="text" class="edit" name="address" id="interactive__comment_address" size="50" tabindex="4" value="<?php echo hsc($_REQUEST['address'])?>" />
                </label>
              </div>
        <?php
        }
        ...
        if ($this->getConf('adminimport') && ($this->helper->isInteractiveMod())) {
        ?>
              <div class="comment_date">
                <label class="block" for="interactive__comment_date">
                  <span><?php echo $this->getLang('date') ?>:</span>
                  <input type="text" class="edit" name="date" id="interactive__comment_date" size="50" />
                </label>
              </div>
        <?php
        }
        ...
        ?>
                <div class="comment_text">
                  ...
                  <!-- Fix for disable the toolbar when wikisyntaxok is set to false. See interactive's script.jss -->
                  <?php if($this->getConf('wikisyntaxok')) { ?>
                    <div id="interactive__comment_toolbar" class="toolbar group">
                  <?php } else { ?>
                    <div id="interactive__comment_toolbar_disabled">
                  <?php } ?>
                </div>
                <textarea class="edit<?php if($_REQUEST['comment'] == 'add' && empty($_REQUEST['text'])) echo ' error'?>" name="text" cols="80" rows="10" id="interactive__comment_text" tabindex="5"><?php
                  if($raw) {
                      echo formText($raw);
                  } else {
              ...
              <input class="button comment_submit" id="interactive__btn_submit" type="submit" name="submit" accesskey="s" value="<?php echo $lang['btn_save'] ?>" title="<?php echo $lang['btn_save']?> [S]" tabindex="7" />
              <input class="button comment_preview_button" id="interactive__btn_preview" type="button" name="preview" accesskey="p" value="<?php echo $lang['btn_preview'] ?>" title="<?php echo $lang['btn_preview']?> [P]" />

        <?php if((!$_SERVER['REMOTE_USER'] || $_SERVER['REMOTE_USER'] && !$conf['subscribers']) && $this->getConf('subscribe')) { ?>
              <div class="comment_subscribe">
                <input type="checkbox" id="interactive__comment_subscribe" name="subscribe" tabindex="6" />
                <label class="block" for="interactive__comment_subscribe">
        ...
        <?php } ?>
              ...
              <div id="interactive__comment_preview">&nbsp;</div>
            </div>
          </form>
        </div>
        <?php
    ...
    protected function _button($cid, $label, $act, $jump = false) {
        ...
        $anchor = ($jump ? '#interactive__comment_form' : '' );

        ?>
        <form class="button interactive__<?php echo $act?>" method="get" action="<?php echo script().$anchor ?>">
          <div class="no">
            <input type="hidden" name="id" value="<?php echo $ID ?>" />
            <input type="hidden" name="do" value="show" />
            <input type="hidden" name="comment" value="<?php echo $act ?>" />
    ...
    protected function _notify($comment, &$subscribers) {
        ...
        if (!empty($subscribers)) {
            ...
            foreach($subscribers as $mail => $data) {
                $mailer->bcc($mail);
                if($data['active']) {
                    $replace['UNSUBSCRIBE'] = wl($ID, 'do=interactive_unsubscribe&hash=' . $data['hash'], true, '&');

                    $mailer->subject($subject_notify);
                    $mailer->setBody($notify_text, $replace);
                    $mailer->send();
                } elseif(!$data['active'] && !$data['confirmsent']) {
                    $confirm_replace['SUBSCRIBE'] = wl($ID, 'do=interactive_confirmsubscribe&hash=' . $data['hash'], true, '&');
    ...
    /**
     * Finds out whether there is a interactive section for the current page
     *
     * @param string $title
     * @return bool
     */
    protected function _hasInteractive(&$title) {
        global $ID;
    ...
    protected function _pageTemplate($data) {
        ...
        $id   = $data['id'];
        $user = $_SERVER['REMOTE_USER'];
        $tpl  = io_readFile(DOKU_PLUGIN.'interactive/_template.txt');
    ...
    /**
     * Add interactive plugin version to the indexer version
     ...
     * @param $param
     */
    public function idx_add_interactive(Doku_Event $event, $param) {
    ...
    public function update_comment_status(Doku_Event $event, $param) {
        ...
        $file = metaFN($ID, '.comments');
        $status = ($this->isInteractiveEnabled() ? 1 : 0);
        $title = NULL;
        if (isset($meta['plugin_interactive'])) {
            $status = $meta['plugin_interactive']['status'];
            $title = $meta['plugin_interactive']['title'];
        } else if ($status == 1) {
