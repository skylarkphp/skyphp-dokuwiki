...
/**
 * Class admin_plugin_interactive
 */
class admin_plugin_interactive extends DokuWiki_Admin_Plugin {
    ...
    function handle() {
        ...
        /** @var action_plugin_interactive $action */
        $action =& plugin_load('action', 'interactive');
    ...
    function html() {
        ...
        // slice the needed chunk of interactive pages
        ...
        foreach ($threads as $thread) {
            ...
            ptln('<input type="hidden" name="page" value="interactive" />', 10);
            echo html_buildlist($comments, 'admin_interactive', array($this, '_commentItem'), array($this, '_li_comment'));
            $this->_actionButtons($thread['id']);
        }
        $this->_browseInteractiveLinks($more, $first, $num);

    }

    /**
     * Returns an array of pages with interactive sections, sorted by recent comments
     *
     * @return array
     */
    function _getThreads() {
        ...
        usort($result, array('admin_plugin_interactive', '_threadCmp'));
    ...
    /**
     * Outputs header, page ID and status of a interactive thread
     *
     * @param array $thread
     * @return bool
     */
    ...
    function _getComments(&$thread) {
        ...
        if (!@file_exists($thread['file'])) return false; // no interactive thread at all
    ...
    /**
     * Displays links to older newer interactives
     *
     * @param bool $more
     * @param int  $first
     * @param int  $num
     * @return bool
     */
    function _browseInteractiveLinks($more, $first, $num) {
        global $ID;

        if (($first == 0) && (!$more)) return true;

        $params = array('do' => 'admin', 'page' => 'interactive');
        $last = $first+$num;
