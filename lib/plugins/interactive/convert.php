...
define('DISCUSSION_NS', 'interactive');
...
$files = getInteractivePages();
$n     = 0;

foreach ($files as $file) {
    if (convertInteractivePage($file)) {
        echo $file['id'].'<br />';
        $n++;
    }
}
...
if ($n > 0) {
    ...
    echo '<br />Successfully converted '.$n.' interactive pages to new comments meta files.';
} else {
    echo 'No interactive pages found.';
}

/* ----- Functions ----- */

/**
 * returns a list of all interactive pages in the wiki
 */
function getInteractivePages() {
    global $conf;

    $data = array();
    search($data, $conf['datadir'], 'search_interactivepages', array());
    return $data;
}
 ...
 */
function search_interactivepages(&$data, $base, $file, $type, $lvl, $opts) {
    global $conf;
...
/**
 * this converts individual interactive pages to .comment meta files
 */
function convertInteractivePage($file) {
