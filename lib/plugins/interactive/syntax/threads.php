...
<?php
/**
 * Interactive Plugin, threads component: displays a list of recently active interactives
 * 
...
/**
 * Class syntax_plugin_interactive_threads
 */
class syntax_plugin_interactive_threads extends DokuWiki_Syntax_Plugin {
    ...
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('\{\{threads>.+?\}\}', $mode, 'plugin_interactive_threads');
    }
    ...
    function render($mode, Doku_Renderer $renderer, $data) {
        ...
        $pages = array();
        /** @var helper_plugin_interactive $my */
        if ($my =& plugin_load('helper', 'interactive')) $pages = $my->getThreads($ns, null, $skipEmpty);
        ...
        if ($mode == 'xhtml') {
            ...
            // show form to start a new interactive thread?
            if ($noNewThreadForm !== true) {
            ...
            foreach ($pages as $page) {
                $page['class'] = 'interactive_status'.$page['status'];
                $pagelist->addPage($page);

                $i++;
                if($count != false && $i >= $count) break; // Only display the n interactive threads specified by the count flag
            }
    ...
    /**
     * Show the form to start a new interactive thread
     *
     * @param string $ns
     * @return string
     */
    function _newThreadForm($ns) {
        ...
        return '<div class="newthread_form">'.DOKU_LF.
            '<form id="interactive__newthread_form"  method="post" action="'.script().'" accept-charset="'.$lang['encoding'].'">'.DOKU_LF.
            ...
            DOKU_TAB.DOKU_TAB.'<input class="edit" type="text" name="title" id="interactive__newthread_title" size="40" tabindex="1" />'.DOKU_LF.
