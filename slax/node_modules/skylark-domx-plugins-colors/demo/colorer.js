function initPage($,colors) {
    var colorInputs = $("input[type=color]");
    if (colorInputs.length && !inputTypeColorSupport()) {
        colorInputs.plugin("domx.colors.box",{
    ...
    function toggleButtonState() {
        //var options = colorpickerInputPlugin.options;
        $(".toggleBtn").each(function() {
            $(this).toggleClass("active", !!colorpickerInputPlugin.stating($(this).data("rule")));
        });
    }

    $(document).on("click", ".toggleBtn", function() {
        var option = $(this).data("rule");
        var existing = colorpickerInputPlugin.stating(option);
        colorpickerInputPlugin.stating(option,!existing);
        toggleButtonState();
    });

    var colorpickerInputPlugin =  colors.colorer(colorpickerInput[0],{
        color: "#ECC",
        flat: true,
